...
class FlowCurve():
    def __init__(self,filename=None,data=None,Unit="Pa",E=None,nu=None,tol=1e-5):
        ...
        strain0 = strain[tmp_max_index] + (0-stress[tmp_max_index])/(tangent[tmp_max_index]) # stress=0
        print("strain0: %e dstress/dstrain: %e\n"%(strain0,tangent[tmp_max_index]))
        for i,v in enumerate(stress):
            if dstrain[i] != 0 and strain[i]<strain0: # compressive data
...
def damask_sim(params,geomfile,loadfile,materialfile,sections):
    ...
    if check == 0:
        subprocess.call(shlex.split("postResults --cr f,p %s.spectralOut" %(jobname))) #--co totalshear
        postfn = jobname+".txt"
        ...
        table.head_read()
        table.data_readArray(["4_ln(V)","4_Cauchy"]) #(["Mises(ln(V))","Mises(Cauchy)"])
        # Suggest (["totalshear","Mises(Cauchy)"])

        sim_flow = FlowCurve(data=table.data,Unit='Pa')
        np.savetxt("./postProc/%s_0.dat"%(jobname),sim_flow.data)
        ...
        sim_data_epssig = [[sim_flow.data[i][0],sim_flow.stress[i]] for i in range(sim_data_n)] 
        np.savetxt("./postProc/%s_1.dat"%(jobname),sim_data_epssig)
        return sim_data_acc
    else:
        print("damask failure")
        return [[float("inf"),float("inf")]]

def damask_obj(params0,geomfile,loadfile,materialfile,sections,exp_data,Unit="MPa"): # opt.least_squares
    params = copy.deepcopy(params0)
    #params[2:6]=params0[2:6]*1e6 # MPa->Pa
    #params[7:9]=params0[7:9]*1e6 # bcc
    params[2:4] = params0[2:4]*1e6 # MPa->Pa
    params[5:7] = params0[5:7]*1e6 # fcc
    ...
    if len(obj)!=1:
        ...
        return [float("inf")] #[float("inf")] # will cause ValueError!

def damask_fit(params0,args,bounds,constraints):
    res = opt.least_squares(damask_obj,x0=params0,args=args,bounds=bounds) #,constraints=constraints)
    return res

def M_predict(M,macroparams): # macroparams: Unit MPa
    dot_eps0,m,f1,f2,f3,f4,b1,b2 = macroparams
    if m > 20: #fcc
        params0_lq = [1e-3,         20, f1/(M),    (f1+f2+b1/b2)/(M), 2.0,    ((f1+f2)*f3+b1)/(M**2), b1/(M**2),b2] # 
    else:
        params0_lq = [dot_eps0*M,   m,  f1/(M),    (f1+f2+b1/b2)/(M), 2.0,    ((f1+f2)*f3+b1)/(M**2), b1/(M**2),b2] # 
    #if m > 20: # bcc
    #    params0_lq = [1e-3,         20, f1/(M),    f1/(M),    (f1+f2+b1/b2)/(M), (f1+f2+b1/b2)/(M), 2.0,    ((f1+f2)*f3+b1)/(M**2), b1/(M**2),b2] # 
    #else:
    #    params0_lq = [dot_eps0*M,   m,  f1/(M),    f1/(M),    (f1+f2+b1/b2)/(M), (f1+f2+b1/b2)/(M), 2.0,    ((f1+f2)*f3+b1)/(M**2), b1/(M**2),b2] # 
    
    return np.asarray(params0_lq)
...
def damask_plot(datafile,geomfile,loadfile,comment=None): # datafile Unit="MPa"
    
    flowcurve = FlowCurve(filename=datafile,Unit="MPa")

    jobname = geomfile.split('.')[0] + '_' + loadfile.split('.')[0]
    sim_data = FlowCurve(filename="./postProc/%s_0.dat" %(jobname),Unit="Pa") #np.loadtxt(fname="./postProc/%s_acc.dat"%(jobname))
    sim_accstrain = sim_data.acc_strain #[ d[0] for d in sim_data]
    sim_vonstress = sim_data.von_stress #[ d[1] for d in sim_data]
    ...
    if comment==None:
        ...
        plt.savefig("./postProc/%s_%s.png"%(jobname,comment))
...
def test():
    ...
    damask_plot(datafile,geomfile,loadfile,comment='0')
...
def main():
    datafile = "gamma_tau21.dat"
    geomfile = "20grains8x8x8.geom"
    loadfile = "shearXY.load"
    materialfile = "material.config"
    sections = ["aluminum_phenopowerlaw"]
    flowcurve = FlowCurve(filename=datafile,Unit="MPa")
    params0_de = [1e-3,20,497.12,336.52,22.84,1.0,3421.46,32.50]
    bounds_de = [(0,1),(0,1e4),(0,1e4),(0,1e4),(0,1e4),(0,1),(0,1e5),(0,1e2),]                                                                     
    params_lq = np.asarray([0.918,7e3,60.59,182.58,4.13,0.815,7348.561,295.18])
    ...
    if res.success==True:
        ...
        obj_M = lambda M: sum(np.square(damask_obj(M_predict(M,params_lq),geomfile,loadfile,materialfile,sections,exp_data,Unit="MPa")))
        res_M = opt.brute(obj_M,ranges=[slice(1,4,0.5)],full_output=True,finish=None)
        M = res_M[0]
        params0_lq = M_predict(M,res.x)
        err_M = res_M[1]
        ...
        with open("%s.log"%(jobname),'a') as errfile:
            ...
            for p in params_lq:
                errfile.write(" %e " %(p)) 
                errfile.write(" error: %e MPa " %(res.fun))
            errfile.write(" M: %e " %(M))
            errfile.write(" obj: %e Pa \n" %(err_M))
        # bcc Unit:MPa
        #bounds_lq = ([0, 1,   0,     0,     100,   100,   1,  0,    0,   1],
        #             [0.1, 1e4, 500, 500, 1e3,     1e3,     3,  20e3, 5e3, 1e2],)
        # fcc Unit:MPa
        bounds_lq = ([0, 1,   0,          0,        1,  0,    0,   1],
                     [1, 1e4, 100,      200,        3,  1e6, 1e5, 1e3],)
        
        print("bounds: ")
        print(bounds_lq)
        #bounds_lq = [(0,0.1),(1,1e4),(0,500e6),(0,500e6),(100e6,1e9),(100e6,1e9),(1,3),(0,20e9),(0,5e9),(1,1e2)] for opt.minimze Unit:Pa
        #bounds_lq = [(0,0.1),(1,1e4),(0,500),(0,500),(100,1e3),(100,1e3),(1,3),(0,20e3),(0,5e3),(1,1e2)] Unit:MPa
        ...
        args = (geomfile,loadfile,materialfile,sections,exp_data,"MPa")
