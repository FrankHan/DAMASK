...
subroutine plastic_phenopowerlaw_dotState(Tstar_v,ipc,ip,el)
 ...
 offset_taub = nSlip + nTwin + 2_pInt + nSlip + nTwin
...
! Calculation of dot gamma
     ...
     tau_b = plasticState(ph)%state(offset_taub+j,of)    
     gdot_slip(j) = plastic_phenopowerlaw_gdot0_slip(instance)*0.5_pReal* &
                  ((abs(tau_slip_pos-tau_b)/(plasticState(ph)%state(j,of)))**plastic_phenopowerlaw_n_slip(instance) &
				  *sign(1.0_pReal,tau_slip_pos-tau_b) &
                  +(abs(tau_slip_neg-tau_b)/(plasticState(ph)%state(j,of)))**plastic_phenopowerlaw_n_slip(instance) &
				  *sign(1.0_pReal,tau_slip_neg-tau_b))
...
! calculate the overall hardening based on above
 ...
 slipFamilies2: do f = 1_pInt,lattice_maxNslipFamily
   slipSystems2: do i = 1_pInt,plastic_phenopowerlaw_Nslip(f,instance)
     ...
     ! taudot_b
     plasticState(ph)%dotState(offset_taub+j,of) = plastic_phenopowerlaw_taub_c(instance)*gdot_slip(j) - &
                                                   plastic_phenopowerlaw_taub_d(instance)* &
                                                   plasticState(ph)%state(offset_taub+j,of)*abs(gdot_slip(j))
   enddo slipSystems2
 enddo slipFamilies2
...
function plastic_phenopowerlaw_postResults(Tstar_v,ipc,ip,el)
 ...
 integer(pInt) :: &
   ...
   nSlip,nTwin, &
   o,f,i,c,j,k, &
   index_Gamma,index_F,index_accshear_slip,index_accshear_twin,index_myFamily,offset_taub
 real(pReal) :: &
   tau_slip_pos,tau_slip_neg,tau,tau_b
